...
사이트와 스킬 리소스는 모두 `docs/` 폴더 하나에 있습니다(GitHub Pages가 `docs/`를 배포 소스로 서빙).

- 정적 웹사이트(`docs/index.html`): 원칙 학습 → 프롬프트 조립 → 레퍼런스 탐색
- 번들 스킬(`docs/SKILL.md`): AI 에이전트가 같은 7원칙으로 프롬프트를 설계하게 하는 실행 규칙
- 레퍼런스 라이브러리(`docs/references/`): 31개 카테고리 파일, 160여 개의 큐레이션 프롬프트
...
| 결과물 카테고리 | 10개 | 갤러리 필터 |
| 선별 레퍼런스 | 160여 개 | `docs/references/gallery-*.md` |
...
> 전체: [`references/gallery-anime-and-manga.md`](docs/references/gallery-anime-and-manga.md)
...
> 전체: [`references/gallery-illustration.md`](docs/references/gallery-illustration.md)
...
> 전체: [`references/gallery-gaming.md`](docs/references/gallery-gaming.md)
...
> 전체: [`references/gallery-cinematic-and-animation.md`](docs/references/gallery-cinematic-and-animation.md)
...
> 전체: [`references/gallery-typography-and-posters.md`](docs/references/gallery-typography-and-posters.md)
...
> 전체: [`references/gallery-product-and-food.md`](docs/references/gallery-product-and-food.md)
...
> 전체: [`references/gallery-photography.md`](docs/references/gallery-photography.md)
...
> 전체: [`references/gallery-architecture-and-interior.md`](docs/references/gallery-architecture-and-interior.md)
...
> 전체: [`references/gallery-infographics-and-field-guides.md`](docs/references/gallery-infographics-and-field-guides.md)
...
> 전체: [`references/gallery-ui-ux-mockups.md`](docs/references/gallery-ui-ux-mockups.md)

전체 카테고리 라우팅 인덱스는 [`references/gallery.md`](docs/references/gallery.md)에 있습니다.
...
2. `docs/references/gallery.md`에서 유사 패턴을 찾아 해당 카테고리 파일을 로딩
3. 7블록으로 프롬프트를 조립하고 정본 필드 어휘로 정밀화
4. 이미지 속 글자·다이어그램·UI는 `docs/references/craft.md` 규칙으로 보정
5. 품질 체크리스트를 통과할 때까지 다듬어 복사용 프롬프트로 전달

프롬프트 작성 자체에는 외부 도구가 필요 없습니다. 실제 이미지 생성은 선택 사항이며, 호스트의 이미지 생성 도구나 `docs/scripts/generate.py`(+ `OPENAI_API_KEY`)로 실행할 수 있습니다. API 호출은 과금될 수 있습니다.
...
사이트와 번들 스킬 리소스가 `docs/` 하나에 모여 있습니다. GitHub Pages를 `docs/` 폴더에서 배포하도록 설정하면 그대로 서빙됩니다.
...
레퍼런스 갤러리의 각 프롬프트에는 원저자와 출처(X 등)가 `docs/references/gallery-*.md`에 표기되어 있습니다. `Curated` 표기는 가이드용으로 정리한 예시입니다.

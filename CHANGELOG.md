...
- 레퍼런스 라이브러리: 10개 카테고리 파일(각 10개, 사이트 갤러리 100개와 1:1) + 작성 규칙(`craft.md`) + 변주 레시피(`variation-recipes.md`)
- 품질 도구: 규칙 검증기(`scripts/check_prompt.py`), 생성물 위 글자 후처리 차단 훅(`hooks/block-text-overlay.sh`), 실측 채점(`experiments/`)
- 비개발자 비즈니스 리더용 설치 안내
